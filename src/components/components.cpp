...
loadGeometry(const tinyobj::ObjReader &reader, GLuint &vertexBufferID, GLuint &vertexArrayID, GLuint &triangles) {
    ...
    glEnableVertexAttribArray(0);
    glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, (2 * faceSides + 2) * sizeof(float), (void *)(3 * sizeof(float)));
    glEnableVertexAttribArray(1);
    glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, (2 * faceSides + 2) * sizeof(float), (void *)(6 * sizeof(float)));
...
renderable::renderable(const std::string &model, const std::string &vertex, const std::string &fragment) {
    ...
    GLuint vertexBufferID, vertexArrayID, triangles;
    ...
    this->textureID = textures.begin()->second; // todo(arlyon) support multiple textures
