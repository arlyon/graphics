...
bool loadTextures(const tinyobj::ObjReader &reader, std::map<std::string, GLuint> &textures) {
    ...
    for (auto &material : materials) {
        ...
        uint8_t *image = stbi_load(material.diffuse_texname.c_str(), &image_width, &image_height, &components,
                                   STBI_default);
        if (!image) {
        ...
        if (components == 3)
            ...
            std::cerr << "Unable to load texture with " << components << " components: " << material.diffuse_texname
                      << std::endl;
            exit(1);
        }
